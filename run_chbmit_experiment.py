"""
Optimized Script to Run CHB-MIT Experiment
...
def main():
    ...
    print("="*80)
    print(" GenEEG: CHB-MIT Dataset Experiment (Optimized)")
    print("="*80)

    # Skip redundant data loading - pipeline will load it
    # This avoids loading data multiple times
    ...
    logger.info("  Expected time: ~1.5-2 hours per fold")
    logger.info("  Total estimated time: ~30-45 hours for 24 patients")

    try:
        from pipeline.cl_lopo_pipeline import main_cl_lopo_validation
        
        logger.info("Starting full CL-LOPO validation...")
        logger.info(f"  Dataset: CHB-MIT (24 patients)")
        logger.info(f"  Device: {device}")
        logger.info(f"  Output: ./outputs/cl_lopo_chbmit")
        
        # Run the pipeline with all optimizations
        results = main_cl_lopo_validation(
            data_root_dir=DatasetConfig.CHBMIT_ROOT_DIR,
            output_dir='./outputs/cl_lopo_chbmit',
            device=str(device),
            dataset_name='chbmit',
            max_patients=None  # Process all patients (set to 3 for quick testing)
        )
    ...
    print("="*80)
    print(f"\nResults saved to: ./outputs/cl_lopo_chbmit/")
